...
PosixErrorOr<std::vector<ProcMountsEntry>> ProcSelfMountsEntries() {
  ...
  for (const std::string& line : lines) {
    ...
    if (fields.size() != 6) {
      return PosixError(
          EINVAL, absl::StrFormat("Not enough tokens, got %d", fields.size()));
    }
    ...
    ASSIGN_OR_RETURN_ERRNO(entry.fsck, Atoi<uint32_t>(fields[4]));
  ...
  for (const std::string& line : lines) {
    ...
    if (fields.size() < 10 || fields.size() > 11) {
      return PosixError(EINVAL,
                        absl::StrFormat("Unexpected number of tokens, got %d",
                                        fields.size()));
    }
...
absl::flat_hash_map<std::string, std::string>
    ParseMountOptions(std::string mopts) {
