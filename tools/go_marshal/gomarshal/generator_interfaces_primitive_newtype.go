...
func (g *interfaceGenerator) emitMarshallableSliceForPrimitiveNewtype(nt *ast.Ident, slice *sliceAPI) {
	...
	g.inIndent(func() {
		...
		g.emit("dest := dst[:size*count]\n")
		g.emit("gohacks.Memmove(unsafe.Pointer(&dest[0]), unsafe.Pointer(&src[0]), uintptr(len(dest)))")
		g.emitKeepAlive("src")
	...
	g.inIndent(func() {
		...
		g.emit("source := src[:(size*count)]\n")
		g.emit("gohacks.Memmove(unsafe.Pointer(&dst[0]), unsafe.Pointer(&source[0]), uintptr(len(source)))\n")
		g.emitKeepAlive("dst")
		g.emit("return size*count, nil\n")
	})
	g.emit("}\n\n")
