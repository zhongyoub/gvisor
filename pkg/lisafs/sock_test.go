...
func TestReadWrite(t *testing.T) {
	...
	runSocketTest(t, func(sock *unet.Socket) {
		...
		gotData := make([]byte, n)
		if err := readFrom(sock, gotData, nil); err != nil {
...
func TestFDDonation(t *testing.T) {
	...
	runSocketTest(t, func(sock *unet.Socket) {
		...
		devRandomFD, err := syscall.Open(path3, syscall.O_RDONLY, 0)
		if err != nil {
			syscall.Close(devNullFD)
			syscall.Close(devFD)
			t.Fatalf("open(%s) failed: %v", path2, err)
		}
		if err := writeTo(sock, [][]byte{data}, n, []int{devNullFD, devFD, devRandomFD}); err != nil {
			t.Errorf("writeTo socket failed: %v", err)
		}
	}, func(sock *unet.Socket) {
		gotData := make([]byte, n)
		var fds [3]int
		if err := readFrom(sock, gotData, fds[:]); err != nil {
			t.Fatalf("reading from socket failed: %v", err)
		}
		defer closeFDs(fds[:])
...
func checkMessageReceive(t *testing.T, sock *unet.Socket, wantM MID, wantMsg marshal.Marshallable) {
	gotM, gotB, err := readMessageFrom(sock, nil)
	if err != nil {
