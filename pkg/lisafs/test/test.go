...
type Version struct {
	...
	VersionLen primitive.Uint16
	Version    []byte `marshal:"unaligned"`
}
...
func (v *Version) MarshalBytes(dst []byte) {
	v.MSize.MarshalBytes(dst)
	dst = dst[v.MSize.SizeBytes():]
	v.VersionLen.MarshalBytes(dst)
	dst = dst[v.VersionLen.SizeBytes():]
	copy(dst, v.Version)
}

// UnmarshalBytes implements marshal.Marshallable.UnmarshalBytes.
func (v *Version) UnmarshalBytes(src []byte) {
	v.MSize.UnmarshalBytes(src)
	src = src[v.MSize.SizeBytes():]
	v.VersionLen.UnmarshalBytes(src)
	src = src[v.VersionLen.SizeBytes():]
	if cap(v.Version) < int(v.VersionLen) {
		v.Version = make([]byte, v.VersionLen)
	} else {
		v.Version = v.Version[:v.VersionLen]
	}
	copy(v.Version, src[:v.VersionLen])
}
