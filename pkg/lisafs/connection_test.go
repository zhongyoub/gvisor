...
func runServerClient(t testing.TB, mountPath string, handlers []lisafs.RPCHanlder, clientFn func(c *lisafs.Client)) {
	...
	c, _, err := lisafs.NewClient(clientSocket, mountPath)
	if err != nil {
...
var mountHandler lisafs.RPCHanlder = func(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
	var req lisafs.MountReq
	req.UnmarshalBytes(comm.PayloadBuf(payloadLen))

	if string(req.MountPath) != c.MountPath() {
		log.Warningf("incorrect mount path found in request: expected %q, got %q", c.MountPath(), req.MountPath)
		return 0, nil, unix.EINVAL
	}

	resp := lisafs.MountResp{
		Root:          lisafs.Inode{ControlFD: 1}, // Dummy inode.
		MaxM:          c.MaxMessage(),
...
func TestUnsupportedMessage(t *testing.T) {
	...
	runServerClient(t, mountPath, serverHandlers[:], func(c *lisafs.Client) {
		if err := c.SndRcvMessage(unsupportedM, 0, nil, nil, nil); err != unix.EOPNOTSUPP {
...
func TestStress(t *testing.T) {
	...
	var dynamicMsgHandler lisafs.RPCHanlder = func(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
...
func BenchmarkSendRecv(b *testing.B) {
	...
	serverHandlers[lisafs.Mount] = mountHandler
	serverHandlers[lisafs.Channel] = lisafs.ChannelHandler
	serverHandlers[versionM] = func(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
	...
	runServerClient(b, mountPath, serverHandlers[:], func(c *lisafs.Client) {
		for i := 0; i < b.N; i++ {
			if err := c.SndRcvMessage(versionM, uint32(sendV.SizeBytes()), sendV.MarshalBytes, recvV.UnmarshalBytes, nil); err != nil {
