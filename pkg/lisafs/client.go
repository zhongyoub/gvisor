...
func (c *Client) createChannel() (*channel, error) {
	...
	var fds [2]int
	if err := c.SndRcvMessage(Channel, 0, nil, chanResp.UnmarshalBytes, fds[:]); err != nil {
		return nil, err
	}
	if fds[0] < 0 || fds[1] < 0 {
		closeFDs(fds[:])
...
func checkResponse(reqM MID, respM MID, comm Communicator) error {
	if respM == Error {
		var resp ErrorRes
		resp.UnmarshalBytes(comm.PayloadBuf(uint32(resp.SizeBytes())))
...
//
// Note that the function arguments intentionally accept the functions
// marshal.Marshallable.MarshalBytes and marshal.Marshallable.UnmarshalBytes
// instead of directly accepting marshal.Marshallable interface. Even though
// just accepting marshal.Marshallable is cleaner, it leads to a heap
// allocation (even if that interface variable itself does not escape).
// In other words, implicit conversion to an interface leads to an allocation.
