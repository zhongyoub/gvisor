...
func (ch *channel) marshalReq(m MID, reqSize uint32, reqMarshal func(dst []byte)) (uint32, error) {
	...
	if reqMarshal != nil {
		...
		if dataLen > MaxMessageSize {
			log.Warningf("message %d has payload which is too long: %d bytes", m, reqSize)
			return 0, unix.EIO
		}
		reqMarshal(ch.PayloadBuf(reqSize))
	}
	return dataLen, nil
}

func (ch *channel) rcvMsg(dataLen uint32, fds []int) (MID, uint32, error) {
	...
	header.UnmarshalBytes(ch.data.Data())
