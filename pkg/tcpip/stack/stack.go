...
type TCPCubicState struct {
	...
	// WMax is the value of the congestion window at the time of the last congestion event.
	WMax float64

	// T is the time when the current congestion avoidance was entered.
	T time.Time `state:".(unixTime)"`

	// TimeSinceLastCongestion denotes the time since the current congestion
	// avoidance was entered.
	TimeSinceLastCongestion time.Duration

	// C is the cubic constant as specified in RFC8312, page 11.
	C float64

	// K is the time period (in seconds) that the above function takes to increase the
	// current window size to WMax if there are no further congestion
	...
	// WC is window computed by CUBIC at time TimeSinceLastCongestion. It's calculated using the
	// formula:
	//
	//  WC(TimeSinceLastCongestion) = C*(t-K)^3 + WMax (Eq. 1)
	WC float64

	// WEst is the window computed by CUBIC at time TimeSinceLastCongestion+RTT i.e
	// WC(TimeSinceLastCongestion+RTT).
	WEst float64
}
...
type TCPRACKState struct {
	// XmitTime is the transmission timestamp of the most recent acknowledged segment.
	...
	// RTT is the round trip time of the most recently delivered packet on the
	// connection (either cumulatively acknowledged or selectively
	// acknowledged) that was not marked invalid as a possible spurious
	// retransmission.
	RTT time.Duration

	// Reord is true iff reordering has been detected on this
	// connection.
	Reord bool
...
type TCPFastRecoveryState struct {
	// Active if true indicates the endpoint is in fast recovery. The following fields
	...
	// It exists to avoid attacks where the receiver intentionally sends duplicate
	// acks to artificially inflate the sender's cwnd.
	MaxCwnd int
...
type TCPReceiverState struct {
	...
	// the "largest" sequence value that the receiver has announced to
	// its peer that it's willing to accept. This may be different than
	// RcvNxt + (last advertised receive window) if the receive window is reduced; in that case we
...
type TCPSenderState struct {
	...
	// Ssthresh is the threshold between slow start and congestion avoidance.
	Ssthresh int
	...
	// Outstanding is the number of packets that have been sent but
...
type TCPEndpointState struct {
	...
	// RcvBufState contains information about the state of the endpoint's receive socket buffer.
	RcvBufState TCPRcvBufState

	// SndBufState contains information about the state of the endpoint's send socket buffer.
