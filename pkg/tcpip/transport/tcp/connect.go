...
func (e *endpoint) drainSendQueueLocked() *segment {
	first := e.sndBuf.mu.sndQueue.Front()
	if first != nil {
		e.snd.writeList.PushBackList(&e.sndBuf.mu.sndQueue)
		e.sndBuf.mu.SndBufInQueue = 0
	}
	return first
}
...
func (e *endpoint) transitionToStateEstablishedLocked(h *handshake) {
	...
	// tuning adjustment.
	e.rcvList.mu.RcvAutoParams.PrevCopiedBytes = int(h.rcvWnd)
	e.rcvList.mu.Unlock()
		...
		},
		{
			...
			f: func() tcpip.Error {
				...
				if n&notifyMTUChanged != 0 {
					e.sndBuf.mu.Lock()
					count := e.sndBuf.mu.PacketTooBigCount
					e.sndBuf.mu.PacketTooBigCount = 0
					mtu := e.sndBuf.mu.SndMTU
					e.sndBuf.mu.Unlock()
