...
func (r *receiver) updateRTT() {
	...
	r.ep.rcvList.mu.Lock()
	if r.ep.rcvList.mu.RcvAutoParams.RTTMeasureTime.IsZero() {
		// New measurement.
		r.ep.rcvList.mu.RcvAutoParams.RTTMeasureTime = time.Now()
		r.ep.rcvList.mu.RcvAutoParams.RTTMeasureSeqNumber = r.RcvNxt.Add(r.rcvWnd)
		r.ep.rcvList.mu.Unlock()
		return
	}
	if r.RcvNxt.LessThan(r.ep.rcvList.mu.RcvAutoParams.RTTMeasureSeqNumber) {
		r.ep.rcvList.mu.Unlock()
		return
	}
	rtt := time.Since(r.ep.rcvList.mu.RcvAutoParams.RTTMeasureTime)
	...
	// measurement of RTT.
	if r.ep.rcvList.mu.RcvAutoParams.RTT == 0 || rtt < r.ep.rcvList.mu.RcvAutoParams.RTT {
		r.ep.rcvList.mu.RcvAutoParams.RTT = rtt
	}
	r.ep.rcvList.mu.RcvAutoParams.RTTMeasureTime = time.Now()
	r.ep.rcvList.mu.RcvAutoParams.RTTMeasureSeqNumber = r.RcvNxt.Add(r.rcvWnd)
	r.ep.rcvList.mu.Unlock()
...
func (r *receiver) handleRcvdSegmentClosing(s *segment, state EndpointState, closed bool) (drop bool, err tcpip.Error) {
	r.ep.rcvList.mu.Lock()
	rcvClosed := r.ep.rcvList.mu.RcvClosed || r.closed
	r.ep.rcvList.mu.Unlock()
