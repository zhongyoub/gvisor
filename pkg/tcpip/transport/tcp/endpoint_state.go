...
func (e *endpoint) Resume(s *stack.Stack) {
	...
	case StateInitial, StateBound, StateListen, StateConnecting, StateEstablished:
		...
		if err := e.stack.TransportProtocolOption(ProtocolNumber, &rs); err == nil {
			if e.rcvList.mu.RcvBufSize < rs.Min || e.rcvList.mu.RcvBufSize > rs.Max {
				panic(fmt.Sprintf("endpoint.rcvList.mu.RcvBufSize %d is outside the min and max allowed [%d, %d]", e.rcvList.mu.RcvBufSize, rs.Min, rs.Max))
			}
		}
	}
