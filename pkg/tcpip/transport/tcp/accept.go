...
func (l *listenContext) createConnectingEndpoint(s *segment, iss seqnum.Value, irs seqnum.Value, rcvdSynOpts *header.TCPSynOptions, queue *waiter.Queue) (*endpoint, tcpip.Error) {
	...
	n.rcvList.mu.RcvBufSize = int(l.rcvWnd)
	...
	n.rcvList.mu.RcvAutoParams.PrevCopiedBytes = n.initialReceiveWindow()
...
func (l *listenContext) startHandshake(s *segment, opts *header.TCPSynOptions, queue *waiter.Queue, owner tcpip.PacketOwner) (*handshake, tcpip.Error) {
	...
	if err := ep.stack.RegisterTransportEndpoint(ep.effectiveNetProtos, ProtocolNumber, ep.TransportEndpointInfo.ID, ep, ep.boundPortFlags, ep.boundBindToDevice); err != nil {
		ep.mu.Unlock()
		ep.Close()
...
func (e *endpoint) reserveTupleLocked() bool {
	dest := tcpip.FullAddress{Addr: e.TransportEndpointInfo.ID.RemoteAddress, Port: e.TransportEndpointInfo.ID.RemotePort}
...
func (e *endpoint) handleListenSegment(ctx *listenContext, s *segment) tcpip.Error {
	...
	case (s.flags & header.TCPFlagAck) != 0:
		...
		// Register new endpoint so that packets are routed to it.
		if err := n.stack.RegisterTransportEndpoint(n.effectiveNetProtos, ProtocolNumber, n.TransportEndpointInfo.ID, n, n.boundPortFlags, n.boundBindToDevice); err != nil {
			n.mu.Unlock()
			n.Close()
