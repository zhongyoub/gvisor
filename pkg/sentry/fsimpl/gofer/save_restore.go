...
func (d *dentry) prepareSaveRecursive(ctx context.Context) error {
	...
	if d.fs.opts.lisaEnabled {
		if d.readFileLisa.Ok() || d.writeFileLisa.Ok() {
			d.fs.savedDentryRW[d] = savedDentryRW{
				read:  d.readFileLisa.Ok(),
				write: d.writeFileLisa.Ok(),
			}
		}
	} else {
...
func (fs *filesystem) CompleteRestore(ctx context.Context, opts vfs.CompleteRestoreOptions) error {
	...
	fs.fdsToClose = make([]lisafs.FDID, 0, fdCacheSize)
...
func (d *dentry) restoreFileLisa(ctx context.Context, inode *lisafs.Inode, opts *vfs.CompleteRestoreOptions) error {
	...
	d.inoKey = inoKey{dev: inode.Stat.Dev, ino: inode.Stat.Ino}
	d.fs.inoMu.Lock()
...
func (d *dentry) restoreRecursive(ctx context.Context, opts *vfs.CompleteRestoreOptions) error {
	if d.fs.opts.lisaEnabled {
		var inodeArr [1]lisafs.Inode
		inodes, err := d.fs.walkLisa(ctx, d.parent.controlFD, []string{d.name}, inodeArr[:])
		if err != nil {
			return err
		}
		if len(inodes) == 0 {
		...
		if err := d.restoreFileLisa(ctx, &inodes[0], opts); err != nil {
			return err
		}
	} else {
...
func (fd *specialFileFD) completeRestore(ctx context.Context) error {
	...
	var err error
	if d.fs.opts.lisaEnabled {
		h, err = d.fs.openHandleLisa(ctx, d.controlFD, fd.vfsfd.IsReadable(), fd.vfsfd.IsWritable(), false /* trunc */)
	} else {
