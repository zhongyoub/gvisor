...
func (d *dentry) getDirents(ctx context.Context) ([]vfs.Dirent, error) {
	...
	if !d.isSynthetic() {
		...
		d.handleMu.RLock()
		if (d.readFile.isNil() && !d.fs.opts.lisaEnabled) || (!d.readFileLisa.Ok() && d.fs.opts.lisaEnabled) {
		...
		for {
			if d.fs.opts.lisaEnabled {
				...
				lisafsDs, err := d.fs.getdents64Lisa(ctx, d.readFileLisa, countLisa)
				if err != nil {
					d.handleMu.RUnlock()
					return nil, err
				}
